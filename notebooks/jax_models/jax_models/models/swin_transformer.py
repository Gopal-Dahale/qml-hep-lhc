...
from notebooks.jax_models.jax_models.layers import PatchEmbed, TransformerMLP, DropPath, AdaptiveAveragePool1D
from notebooks.jax_models.jax_models.models.helper import load_trained_params, download_checkpoint_params
from notebooks.jax_models.jax_models.models.model_registry import register_model
...
pretrained_cfgs = {
    "swin-tiny-224":
        "https://github.com/DarshanDeshpande/jax-models/releases/download/v0.1-swin/swin_tiny_224_1k.weights",
    "swin-small-224":
        "https://github.com/DarshanDeshpande/jax-models/releases/download/v0.1-swin/swin_small_224_1k.weights",
    "swin-base-224":
        "https://github.com/DarshanDeshpande/jax-models/releases/download/v0.1-swin/swin_base_224_22k.weights",
    "swin-base-384":
        "https://github.com/DarshanDeshpande/jax-models/releases/download/v0.1-swin/swin_base_384_22k.weights",
    "swin-large-224":
        "https://github.com/DarshanDeshpande/jax-models/releases/download/v0.1-swin/swin_large_224_22k.weights",
    "swin-large-384":
        "https://github.com/DarshanDeshpande/jax-models/releases/download/v0.1-swin/swin_large_384_22k.weights",
}
...
def window_partition(x, window_size):
    ...
    windows = jnp.reshape(jnp.transpose(x, (0, 1, 3, 2, 4, 5)),
...
def window_reverse(windows, window_size, height, width):
    ...
    x = jnp.reshape(jnp.transpose(x, (0, 1, 3, 2, 4, 5)),
...
class WindowAttention(nn.Module):
    ...
    def get_rel_pos_index(self):
        ...
        relative_indices = flatten_indices[:, :,
                                           None] - flatten_indices[:, None, :]
        relative_indices = jnp.transpose(relative_indices, (1, 2, 0))
        relative_indices = relative_indices.at[:, :, 0].add(
            self.window_size[0] - 1)
        relative_indices = relative_indices.at[:, :, 1].add(
    ...
    def __call__(self, inputs, mask=None, deterministic=None):
        ...
        relative_pos_index = self.variable("relative_position_index",
                                           "relative_position_index",
                                           self.get_rel_pos_index)

        batch, n, channels = inputs.shape
        qkv = nn.Dense(self.dim * 3, use_bias=self.use_bias, name="qkv")(inputs)
        qkv = qkv.reshape(batch, n, 3, self.num_heads,
        ...
        if mask is not None:
            att = jnp.reshape(
                att,
                (batch // mask.shape[0], mask.shape[0], self.num_heads, n, n))
...
class SwinBlock(nn.Module):
    ...
    def get_att_mask(self, shift_size, window_size, height, width):
        if shift_size > 0:
            ...
            mask_windows = jnp.reshape(mask_windows,
    ...
    def __call__(self, inputs, deterministic=None):
        ...
        x_windows = jnp.reshape(x_windows,
                                (-1, window_size * window_size, channels))
        ...
        att_windows = jnp.reshape(att_windows,
                                  (-1, window_size, window_size, channels))
        ...
        mlp = TransformerMLP(self.dim * self.mlp_ratio,
                             self.dim,
                             dropout=self.dropout,
                             name="mlp")(norm, deterministic)
    ...
    def __call__(self, x, deterministic=None):
        ...
        for i in range(self.depth):
            x = SwinBlock(
                ...
                self.dropout,
                self.att_dropout,
                self.drop_path[i] if isinstance(self.drop_path,
                                                (list,
                                                 tuple)) else self.drop_path,
                name=f"blocks{i}",
            )(x, deterministic)
...
@register_model
def swin_tiny_224(attach_head=True,
                  num_classes=1000,
                  dropout=0.0,
                  pretrained=False,
                  download_dir=None):
    ...
    if pretrained:
        file_path = download_checkpoint_params(pretrained_cfgs["swin-tiny-224"],
                                               download_dir)
...
@register_model
def swin_small_224(attach_head=True,
                   num_classes=1000,
                   dropout=0.0,
                   pretrained=False,
                   download_dir=None):
...
def swin_base_224(
    ...
    if pretrained:
        file_path = download_checkpoint_params(pretrained_cfgs["swin-base-224"],
                                               download_dir)
        params = load_trained_params(file_path)
        return model, params
...
def swin_base_384(
    ...
    if pretrained:
        file_path = download_checkpoint_params(pretrained_cfgs["swin-base-384"],
                                               download_dir)
        params = load_trained_params(file_path)
        return model, params
